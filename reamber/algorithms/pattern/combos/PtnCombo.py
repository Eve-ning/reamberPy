...
class PtnCombo(_PtnCChordStream,
    ...
    def combinations(self, size=2, flatten=False, make_size2=False,
        ...
        **Type Filter**
        ...
        """ We loop through the chunks here, finding all permutations of each 
        chunk
        ...
        e.g.
        [0][1][3 4] -> [0][1][3] + [0][1][4] ---[makeSize2 == True]
            ---> [0][1] + [1][3] + [0][1] + [0][4]
        ...
        for chunk in chunks:
            ...
            for i, combo in enumerate(combos):
                for j, col, offset, type_ in \
                    zip(range(size), combo['column'], combo['offset'],
                        combo['type']):
        ...
        if not make_size2:
            return np.asarray(
                [i for j in combo_list for i in j]) if flatten else combo_list
        else:
            # This will make pairs out of (>2)-size combos by
            # iterating through the combo pairs.
            ...
            for i in s[1:]: i.dtype.names = ['column0', 'column1', 'offset0',
                                             'offset1', 'type0', 'type1']
            return np.hstack(s)
