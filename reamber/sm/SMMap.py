...
class SMMap(Map[SMNoteList, SMHitList, SMHoldList, SMBpmList], SMMapMeta):
    ...
    def _read_notes(self, measures: List[List[str]]):
        ...
        def _expand(objs, cls):
            ...
            for key_, i_ in enumerate(objs):
                ...
                objs_.extend([cls(offset, key_)
                              for offset in tm.offsets(*list(zip(*[(obj.measure, obj.beat, obj.snap) for obj in i_])))])
            return objs_
        ...
        def _expand_hold(objs, cls):
            ...
            for key_, i_ in enumerate(objs):
                if not i_: continue
                head = tm.offsets(*list(zip(*[(obj.head.measure, obj.head.beat, obj.head.snap,) for obj in i_])))
                tail = tm.offsets(*list(zip(*[(obj.tail.measure, obj.tail.beat, obj.tail.snap,) for obj in i_])))
