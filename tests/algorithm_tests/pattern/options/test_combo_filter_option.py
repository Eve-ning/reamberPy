...
def test_combo_repeat():
    assert np.all(
        PtnFilterCombo.create(
            combos=[[1, 1, 1]], keys=3, options=PtnFilterCombo.Option.REPEAT
        ).ar
...
def test_combo_h_mirror():
    assert np.all(
        PtnFilterCombo.create(
            combos=[[0, 0, 0]], keys=3, options=PtnFilterCombo.Option.HMIRROR
        ).ar
...
def test_combo_v_mirror():
    assert np.all(
        PtnFilterCombo.create(
            combos=[[0, 2, 2]], keys=3, options=PtnFilterCombo.Option.VMIRROR
        ).ar
