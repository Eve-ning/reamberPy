...
class TestOsuHitList(unittest.TestCase):
    ...
    def test_columns(self):
        self.assertListEqual([0, 1, 2, 3], self.hold_list.columns.to_list())

    def test_lengths(self):
        self.assertListEqual([1000, 1000, 1000, 1000], self.hold_list.lengths.to_list())

    def test_lengths_change(self):
        self.hold_list.lengths *= 2
        self.assertListEqual([2000, 2000, 2000, 2000], self.hold_list.lengths.to_list())
        self.assertListEqual([2000, 3000, 4000, 5000], self.hold_list.tail_offsets.to_list())

    def test_samples(self):
        self.assertListEqual([S0, S1, S2, S3], self.hold_list.sample_sets.to_list())
        self.assertListEqual([S0, S1, S2, S3], self.hold_list.addition_sets.to_list())
        self.assertListEqual([S0, S1, S2, S3], self.hold_list.hitsound_sets.to_list())
        self.assertListEqual([S0, S1, S2, S3], self.hold_list.custom_sets.to_list())

    def test_read(self):
        holds = OsuHoldList.read(self.strings, keys=4)
        self.assertListEqual([0, 1, 2, 3], holds.columns.to_list())
        self.assertListEqual([0, 1000, 2000, 3000], holds.offsets.to_list())
