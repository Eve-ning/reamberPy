...
def bpm_strings(offsets, bpm_bpms, bpm_metronomes, hitsound_sets, volumes):
    return [
        ...
        for offset, bpm, metronome, hitsound_set, volume in zip(
            offsets, bpm_bpms, bpm_metronomes, hitsound_sets, volumes
        )
    ]
...
def hit_strings(offsets, columns, hitsound_sets, volumes):
    ...
    return [
        ...
        for offset, column, hitsound_set, volume in zip(
            offsets, columns, hitsound_sets, volumes
        )
    ]
...
def hold_strings(offsets, columns, hold_lengths, hitsound_sets, volumes):
    ...
    return [
        ...
        f"{hitsound_set}:{hitsound_set}:{hitsound_set}:{volume}:"
        for offset, column, hold_length, hitsound_set, volume in zip(
            offsets, columns, hold_lengths, hitsound_sets, volumes
        )
    ]
